...
sealed trait EitherTree[T] {
  infix def map[U](f: T => U): EitherTree[U]
  def flatMap[U](f: T => EitherTree[U]): EitherTree[U]
}

case class Right[T](v: T) extends EitherTree[T]  {
  infix def map[U](f: T => U): Right[U] = Right(f(v))

  override def flatMap[U](f: T => EitherTree[U]): EitherTree[U] = f(v)
}

sealed trait Left[T] extends EitherTree[T] {
  infix def map[U](f: T => U): Left[U]
  infix def flatMap[U](f: T => EitherTree[U]): Left[U]
}

case class LeftLeaf[T](cause: Exception) extends Left[T] {
  infix def map[U](f: T => U): LeftLeaf[U] = LeftLeaf[U](cause)

  infix override def flatMap[U](f: T => EitherTree[U]): LeftLeaf[U] = LeftLeaf[U](cause)
}

case class LeftTree[T](first: Left[T], second: Left[T]) extends Left[T] {
  infix def map[U](f: T => U): LeftTree[U] = LeftTree[U](first.map(f), second.map(f))
