...
abstract class Functor(
  ...
  /**
    ...
    * @return g compose this : X → Z - composition of this functor with functor g
    */
  def compose(g: Functor): Option[Functor] = {
    if (d1 != g.d0) None else Some {
      ...
      new Functor(g.tag + " compose " + this.tag, f.d0, g.d1) {
        comp: Functor ⇒
  ...
  /**
    ...
    * and there is a g: x1 → x2, then f2 = F(g) compose f1 : y → F(x2) is also in this collection.
    *
  ...
  /**
    ...
    * for all objects x of domain category, such that F(f) compose cx1 = cx2 for f:x1 → x2.
    *
